...
## 🧪 Ручное тестирование (2026-08-14, вечер) — новые находки

> Пользователь гонял сборку вручную. Зафиксированы баги/пожелания; часть уже закрыта.

### ✅ ИСПРАВЛЕНО: маски/промпт/денойз сбрасывались при отправке генерации

**Корень:** Anima-кламп в `GenerateButton.svelte:219-224` мутировал `generation.width/height` (до 1024²), а `$effect` в `CanvasEditor.svelte:27-45` следит за этими полями и при расхождении зовёт `initCanvas()` — который сносит ВСЕ слои (маски + per-mask prompt/denoise).

**Фикс:** кламп перенесён в `generation.toParams()` (применяется к возвращаемым `params`, не к `this.width/height`); мутация в GenerateButton удалена. Следствие — вероятно, чинит также «Accept → пустые слои» и «превью не та картинка» (результат приходил на чужом размере после ресета канваса). **Требуется повторный ручной тест.**

### 🔴 ОТКРЫТО: результат цепи из нескольких масок не показывается

`runMaskInpaintChain` (`maskInpaintChain.ts`) гонит последовательный инпейнт; финальный шаг НЕ супрессится (`isFinalOutput`), идёт через `finalizeOutputImages` → `prepareLatestInpaintResult` → `setPendingInpaintResult`. Дисплей должен сработать. Возможные причины (исследуется): гонка `waitForPromptOutput`/`waitForPromptCompletion` (цепь ждёт выход сама) с WebSocket-путём `output_image`, либо `progress.clearPromptOutput` чистит раньше времени, либо гард `inpaintSourceVersion`. Диагностика продолжается.

### 🔴 ОТКРЫТО (нужен репро-чек после Anima-фикса)

- **Accept → все слои пустые.** `applyInpaintResult` → `clearMask` + `initCanvas` (by design: новая база, свежая маска). Результат должен быть виден как фон (`preparedInpaintPreviewUrl` → `effectiveReferenceImage`). Если после Anima-фикса фон всё ещё пуст — баг в рендере фона.
- **Превью не та картинка.** Вероятно следствие Anima-ресета канваса (результат на чужом размере). После фикса — ре-тест.

### 🟡 Фичи (требуют дизайн-решения)

1. **Accept** — картинка должна чисто заменять input, ИЛИ сохраняться полноценным слоем (полная картинка, не только регион). Сейчас «Apply as Layer» сохраняет ТОЛЬКО замаскированный регион (alpha).
2. **Полный слой** — вариант сохранить результат целиком (не обрезая по маске).
3. **Мягкие края** — растровый результат имеет резкие (binary) края → нужен feather/мягкий alpha-переход.
4. **Нижняя лента (история)** — сделать удобнее (контекстное меню, крупнее, действия).
