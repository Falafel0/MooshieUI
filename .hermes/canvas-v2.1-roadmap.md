...
| 2 | «Clear all» сбрасывает слои и маски (`clearPreparedInputs` → `restoreOriginalInpaintSource` → `clearMask`+`initCanvas`) | 🔴 → часть 1.3 |
| 3 | Результат цепи из нескольких масок не показывается (рантайм-гонка `waitForPromptOutput`/WebSocket) | 🔴 диагностируется |
| 4 | Accept → пустые слои (проверить после Anima-фикса) | 🟡 ре-тест |
| 5 | Превью не та картинка (проверить после Anima-фикса) | 🟡 ре-тест |
| 6 | Мягкие края растрового результата (feather вместо резких) | 🟡 → часть 1.1/1.2 |
| 7 | Accept сохраняет только регион, не полную картинку | 🟡 → часть 1.2 |

---

## 3. Фазы выполнения

**Фаза A — закрыть bugfix-backlog (текущее):**
A1. «Clear all» не сбрасывает слои/маски (переписать `clearPreparedInputs` — часть 1.3).
A2. Диагностировать и починить «результат цепи масок не показывается» (runtime, лог devtools).
A3. Ре-тест Accept/превью после Anima-фикса; при остатке — баг в рендере фона.
A4. Feather для растрового результата (мягкий alpha-переход по краю маски).

**Фаза B — перенести inpaint-настройки на слои (1.1):**
B1. Расширить `CanvasLayer` полным набором inpaint-полей + fallback на глобальные дефолты.
B2. Панель настроек слоя (UI в LayerItem/LayerPanel).
B3. `getMaskInpaintSteps`/`runMaskInpaintChain`/`GenerateButton` читают per-layer.

**Фаза C — input image опциональна + всё на растрах (1.2):**
C1. Снять guard обязательного `inputImage` в canvas-режиме.
C2. База = композит raster-слоёв; txt2img при пустых растрах.
C3. Accept/apply — полная картинка (не только регион) как опция.

**Фаза D — явные кнопки очистки, без авто-сброса (1.3):**
...
**Фаза E — dimension-панель (1.4) + вкладки (1.5).**
