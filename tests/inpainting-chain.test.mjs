...
function setup(mode='inpainting') {
  Object.assign(fixture.generation, { mode, isAnima:false, differentialDiffusion:false, loras:[], toParams: (options={}) => ({mode,input_image:'source.png',positive_prompt:'shared',seed:'123',width:64,height:64,facefix_enabled:true,detail_segments:[{text:'detail'}],inpaint_settings:{mask_blur:4},positive_regions:options.regionalSelectionsOverride??[]}) });
...
test('mask passes keep global negative prompt because local negative is spatial conditioning',async()=>{const {calls,callbacks,regions}=setup();regions[0].negativePrompt='blurry';await runRegionalInpaintChain(regions,callbacks);assert.equal(calls[0].params.negative_prompt,undefined);assert.equal(calls[1].params.negative_prompt,undefined);});
