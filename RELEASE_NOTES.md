...
- Text-to-image prompt regions use spatial mask conditioning for Anima instead of sequential inpaint passes. A saved inpaint-chain preference no longer redirects a normal text-to-image generation.
- A completed result is never replaced by its last live frame. When a result has no display copy, the final JXL is transcoded on demand so the finished image is what stays on screen.

### Layers and Photopea
- Layer add, delete, duplicate and reorder, plus visibility, opacity, rename and lock changes, are covered by the shared undo/redo history. Undoing a delete restores the same layer with its pixels.
- A Photopea load failure now reports a timeout with retry and open-in-browser actions instead of hanging on the embedded frame. External network or Cloudflare challenges can still block the embedded editor.
...
### Fixes and maintenance
- **Imported images now actually change the resolution in NovelAI mode**: loading an img2img source highlighted the matching aspect ratio preset but left width and height at their old values, so the highlight and the request disagreed. The image's ratio is now applied at your current side length, exactly as clicking that preset would, so a 1024x1536 source at a 1024 side becomes 832x1216. Inpainting still adopts the image's exact size for the mask canvas, and a locked resolution ignores the import entirely.
- **Any aspect ratio in NovelAI mode again**: v2.2.5 limited NovelAI mode to NovelAI's own shapes, snapped typed ratios to the nearest one and inferred imported ratios the same way. That was too strict. All eight presets are back on every backend, typed ratios are kept as typed, and imported ratios are reduced to lowest terms. Only the 64px grid and the round-down area rule remain NovelAI-specific, so dimensions always land on the largest 64-multiple pair that stays at or under the current area.
...
### New features
- **Settings sorted into categories**: Settings is now a sidebar of categories that each open their own page, instead of every section floating on one long masonry screen. The search box narrows the sidebar to matching categories, the category you were on is remembered, and on mobile the sidebar becomes a row of chips across the top.
- **OS completion notifications**: a new Notifications section in Settings with an enable toggle and an "only when the window is unfocused" toggle. Desktop uses the native notification plugin, browser mode uses the Web Notifications API. Fires when an image batch or video finishes and on generation errors. ([#659](https://github.com/Mooshieblob1/MooshieUI/pull/659))
- **Random prompt syntax**: `{a|b|c}`, `{2$$a|b|c}`, weighted options and nesting, expanded per generation from a random source seeded by the generation seed so results are reproducible. A dice badge, preview tooltip and reroll live in the prompt inputs. See `docs/RANDOM_PROMPTS.md`. ([#659](https://github.com/Mooshieblob1/MooshieUI/pull/659))
- **Bulk CivitAI scan**: one button in the LoRA gallery hashes every LoRA, checkpoint, diffusion model and embedding and fetches its CivitAI metadata into sidecar files, with progress, cancel and rate limit backoff. Already cached files are skipped unless you re-scan. Works in browser mode for staff accounts. ([#659](https://github.com/Mooshieblob1/MooshieUI/pull/659))
- **Queue panel**: a list icon on the Generate button opens a panel showing the running job and the pending jobs, with move to front, move up, move down, cancel, interrupt and clear pending. ([#659](https://github.com/Mooshieblob1/MooshieUI/pull/659))
- **Style Reference (IP-Adapter and Flux Redux)**: a new section for txt2img, img2img and inpainting that steers the output towards a reference image. Flux.1 dev, schnell and krea use the core Redux nodes; SD1.5, SDXL, Illustrious and Pony use IP-Adapter, installed on demand. Strength, weight type and start/end controls. Coexists with the Anima ReStyler style transfer section. See `docs/STYLE_REFERENCE.md`. ([#659](https://github.com/Mooshieblob1/MooshieUI/pull/659))
- **Make Admin for LAN accounts**: a desktop admin can promote a LAN account to admin (and revoke it again) from the account actions modal, so that account sees the shared desktop gallery from a phone or another remote client. Admin accounts get a badge in the LAN account list, and a token-authenticated admin can no longer change its own role, so a headless server cannot lock itself out. ([#661](https://github.com/Mooshieblob1/MooshieUI/pull/661), fixes [#660](https://github.com/Mooshieblob1/MooshieUI/issues/660))
- **ComfyUI-INT8-Fast loader (W8A8 and ConvRot)**: the model selector gains an INT8 toggle, a ConvRot sub-toggle, an install button for the node pack and a hint when an INT8 file is picked with the toggle off. Supported for Flux 2, Flux 2 Klein, Z-Image, Chroma, Krea 2, Qwen, Anima and Ideogram 4. INT8 on a GGUF file is refused with a clear message. ([#658](https://github.com/Mooshieblob1/MooshieUI/pull/658), fixes [#647](https://github.com/Mooshieblob1/MooshieUI/issues/647))
- **MiniMax H3 custom stack**: a new Custom tier in the H3 video stack lets you pick the diffusion model, text encoder, VAE, audio VAE, turbo LoRA, sampler and scheduler individually, with GGUF models routed through the GGUF loaders. ([#658](https://github.com/Mooshieblob1/MooshieUI/pull/658), fixes [#636](https://github.com/Mooshieblob1/MooshieUI/issues/636))
- **Custom WD tagger folders**: extra tagger model folders can be added and removed from Settings. Models are validated on load and unknown model ids error instead of silently falling back to the default. ([#658](https://github.com/Mooshieblob1/MooshieUI/pull/658), fixes [#617](https://github.com/Mooshieblob1/MooshieUI/issues/617))
- **Manual save mode covers video**: with manual save on, a finished video waits in the preview with a Save to Gallery button instead of being written to the gallery straight away, the same as images. ([#658](https://github.com/Mooshieblob1/MooshieUI/pull/658), fixes [#657](https://github.com/Mooshieblob1/MooshieUI/issues/657))

### Fixes and maintenance
- **NovelAI img2img and inpainting no longer fail with a 500**: the img2img and inpainting tabs upload their source through ComfyUI and keep the returned filename, and in NovelAI mode that filename was sent to NovelAI as the image itself, which it could not decode. Uploads are now read back, fitted to the requested size and sent as the base64 PNG NovelAI expects. The mask is scaled without interpolation so it stays a hard selection. A missing upload now surfaces as a clear app error before any Anlas are spent.
- **Imports never change your resolution**: loading an image into img2img (browse, drop, paste), dropping an image for a whole metadata import, and the NovelAI import modal's Settings pass used to overwrite width and height with the image's size. The resolution is yours: the image's shape is adopted as the aspect ratio at your current side length, so a 1024x1536 source at a 1024 side becomes 832x1216 rather than 1024x1536, and the source is fitted to that. Dropping an image onto the Dimensions section itself still applies its size, because that is you asking for exactly that, and inpainting still sizes the canvas to the image so the mask matches. Switching backends no longer recalculates the dimensions either; they only snap to the 64 grid for NovelAI.
- **Imported images set the aspect ratio, not raw pixels**: loading a 1024x1536 image into img2img showed 1024 : 1536 in the aspect ratio inputs instead of 2 : 3. The suggested ratio is now reduced and snaps to the nearest preset within a small tolerance on either backend, so a 1216x832 image reads 3 : 2. NovelAI mode accepts any aspect ratio; only the 64px grid and the round-down area rule are NovelAI-specific.
- **Send to inpaint uses the right canvas size**: inpaint imports use their own 4 MP cap instead of the 1 MP img2img cap, snapped to multiples of 8, and the canvas is re-initialised when the size differs. ([#658](https://github.com/Mooshieblob1/MooshieUI/pull/658), fixes [#648](https://github.com/Mooshieblob1/MooshieUI/issues/648))
- **Remote uploads behind a JSON-only proxy**: an upload rejected by a proxy that only forwards JSON now shows a message explaining the cause and the fix instead of a raw string, and the setup wizard warns when a remote server's proxy blocks multipart uploads. ([#658](https://github.com/Mooshieblob1/MooshieUI/pull/658), fixes [#649](https://github.com/Mooshieblob1/MooshieUI/issues/649))
- **Hardening pass**: path traversal guard for custom tagger folders, the default admin password "changeme" is refused at server startup, browser auth token storage reads and writes use the same storage, async webserver handlers no longer block on sync filesystem reads, Python nodes log exceptions instead of swallowing them, several dead browser-mode commands removed, a regional prompt schedule base-text bug fixed, unsaved videos kept as a list with a Discard button, and the Flux 2 scheduler dropdown locked. ([#659](https://github.com/Mooshieblob1/MooshieUI/pull/659))
- **Dev server no longer crashes on a Rust rebuild**: on Windows, Vite's file watcher tripped over the freshly linked Rust DLL under `src-tauri/target` and killed `npm run tauri dev` with an EBUSY error. The watcher now ignores `src-tauri` entirely.
- **Dependency updates**: log 0.4.34, flate2 1.1.10, uuid 1.26.0, open 5.4.2, marked 18.0.11 and konva 10.3.2. ([#651](https://github.com/Mooshieblob1/MooshieUI/pull/651)-[#656](https://github.com/Mooshieblob1/MooshieUI/pull/656))
...
- **Fixed an intermittent crash when using prompt-schedule tags with artist tags**: TeaCache's step-reuse optimization compared conditioning-batch statistics across steps without accounting for `<fromto>`/`<from>`/`<to>`/`<range>` tags changing the batch size mid-generation, which could raise a `RuntimeError` when combined with artist tags like `<fromto[0.2]: || (@mitsu \(mitsu art\):1.00)>`. A batch-size change now resets TeaCache's reuse state instead of comparing incompatible shapes.
- **i18n sweep**: replaced hardcoded English strings with translated locale keys across the model picker, prompt inputs, style editor, Model Hub, Settings, Gallery, Animadex character explorer, and the Artist Gallery favourites exporter, so these strings now respect the selected language.
